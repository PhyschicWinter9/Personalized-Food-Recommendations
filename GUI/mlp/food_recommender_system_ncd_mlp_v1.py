...
class HealthAwareMLPRecommender:
    """MLP-based food recommender with health condition awareness"""
    ...
    def load_data(self):
        ...
        try:
            # Look for CSV files in datasets directory
            dataset_folder = './datasets'
            csv_files = glob.glob(os.path.join(dataset_folder, '*.csv'))
            
            if not csv_files:
                # Try current directory if datasets folder doesn't exist
    ...
    def prepare_features(self):
        """Prepare features for MLP model"""
        ...
        self.update_status(f"Prepared {len(self.features)} features for MLP model")
    
    def train_models(self):
        """Train MLP models for different recommendation tasks"""
        ...
        try:
            ...
            # Train MLP models for different health conditions
            self.mlp_models = {}
            
            # General nutritional similarity MLP
            self.update_status("Training general MLP regressor...")
            ...
            # Use overall health score as target for general model
            y_general = self.food_data['Overall_Health_Score']
            general_mlp.fit(X_scaled, y_general)
            self.mlp_models['general'] = general_mlp
            
            # Health-condition-specific MLP models
            health_conditions = ['Diabetes', 'Obesity', 'Hypertension', 'High_Cholesterol']
            
            for condition in health_conditions:
                self.update_status(f"Training MLP model for {condition}...")
                
                # Get condition-specific target
                target_col = f'{condition}_Score'
                y_condition = self.food_data[target_col]
                
                # Create condition-specific MLP
            ...
            self._evaluate_mlp_performance(X_scaled)
            
            self.update_status("MLP models trained successfully!")
            
        except Exception as e:
            self.update_status(f"Error training MLP models: {e}")
    
    def _evaluate_mlp_performance(self, X_scaled):
        """Evaluate MLP model performance"""
        try:
            for model_name, mlp_model in self.mlp_models.items():
                # Get target data
                if model_name == 'general':
                    y_true = self.food_data['Overall_Health_Score']
                else:
                    y_true = self.food_data[f'{model_name}_Score']
                ...
                cv_scores = cross_val_score(mlp_model, X_scaled, y_true, cv=5, scoring='r2')
                ...
                self.stats['model_performance'][model_name] = {
                    ...
                    'loss_curve': mlp_model.loss_curve_ if hasattr(mlp_model, 'loss_curve_') else None
                }
                
                self.update_status(f"{model_name} MLP - R²: {r2:.3f}, CV: {cv_scores.mean():.3f}")
            
        except Exception as e:
            self.update_status(f"Error evaluating MLP performance: {e}")
    
    def get_recommendations(self, user_profile, meal_type='meal', max_recommendations=10):
        """Get personalized food recommendations using MLP"""
        ...
        try:
            ...
            self.update_status(f"Processing {len(candidates)} candidate foods for MLP matching")
            
            # Create user profile vector based on calculated targets
            user_vector = self._create_user_vector(meal_targets)
            
            # Get MLP recommendations based on health conditions
            if health_conditions:
            ...
            self.update_status(f"MLP algorithm found {len(recommendations)} recommendations")
    ...
    def _get_health_condition_recommendations(self, user_vector, health_conditions, candidates, max_recommendations):
        """Get recommendations considering health conditions using MLP"""
        all_recommendations = []
        ...
        for condition in health_conditions:
            if condition in self.mlp_models:
                try:
                    ...
                    # Predict health scores using MLP
                    predicted_scores = self.mlp_models[condition].predict(candidate_features_scaled)
                    
                    # Calculate similarity scores (combine MLP prediction with nutritional similarity)
                    similarity_scores = []
                    for i, candidate_features_row in enumerate(candidate_features_scaled):
                        # Nutritional similarity (Euclidean distance)
                        nutritional_distance = euclidean(scaled_user_vector[0], candidate_features_row)
                        
                        # Health suitability (predicted score - lower is better)
                    ...
                    top_scores = similarity_scores[:min(max_recommendations, len(similarity_scores))]
                    
                    # Create recommendations
                    for score, candidate_idx in top_scores:
                        food = candidates.iloc[candidate_idx]
                        rec = self._create_recommendation_object(food, score, condition, predicted_scores[candidate_idx])
                        all_recommendations.append(rec)
                        
                except Exception as e:
                    self.update_status(f"Error in {condition} MLP: {e}")
                    continue
    ...
    def _get_general_recommendations(self, user_vector, candidates, max_recommendations):
        """Get general nutritional recommendations using MLP"""
        try:
            # Create feature matrix for candidates only
            candidate_features = candidates[self.features].fillna(0)
            ...
            # Predict health scores using general MLP
            ...
            for score, candidate_idx in top_scores:
                food = candidates.iloc[candidate_idx]
                rec = self._create_recommendation_object(food, score, 'general', predicted_scores[candidate_idx])
                recommendations.append(rec)
            
            return recommendations
            
        except Exception as e:
            self.update_status(f"Error in general MLP: {e}")
            return []
    ...
    def _generate_explanation(self, recommendation, targets, health_conditions):
        ...
        # MLP prediction insight
        predicted_score = recommendation['predicted_health_score']
        if predicted_score < 1.0:
            explanations.append("MLP predicts excellent health compatibility")
        elif predicted_score < 2.0:
            explanations.append("MLP predicts good health compatibility")
...
class HealthDrivenMLPFoodRecommenderUI:
    """Modern GUI for the health-driven MLP food recommendation system"""
    
    def __init__(self, master, recommender=None):
        self.master = master
        self.master.title("Health-Driven MLP Food Recommendation System")
        ...
        self.recommender = recommender or HealthAwareMLPRecommender()
    ...
    def create_header(self, parent):
        ...
        ttk.Label(title_frame, text="Health-Driven MLP Food Recommendation System", 
                 style='Title.TLabel').pack(anchor=tk.W)
        ttk.Label(title_frame, text="Multi-layer Perceptron Neural Network + Medical Guidelines", 
    ...
    def create_results_panel(self, parent):
        ...
        self.notebook.add(rec_frame, text="MLP Recommendations")
        
        # Treeview for recommendations
        columns = ('Name', 'Category', 'Calories', 'Protein', 'Carbs', 'Sugar', 'Fiber', 'MLP Score', 'Suitable For')
        ...
        column_widths = {'Name': 150, 'Category': 100, 'Calories': 70, 'Protein': 60, 'Carbs': 60, 
                        'Sugar': 50, 'Fiber': 50, 'MLP Score': 80, 'Suitable For': 120}
        ...
        self.details_text.insert(tk.END, "Select a food item to view detailed nutritional information and MLP analysis.")
    
    def create_charts_panel(self, parent):
        """Create charts panel for visualizations"""
        charts_frame = ttk.LabelFrame(parent, text="Nutritional Analysis & MLP Insights", padding="10")
    ...
    def show_nutrition_calculation(self):
        ...
        try:
            ...
            # Add header
            self.targets_text.insert(tk.END, "PERSONALIZED NUTRITIONAL TARGETS (MLP-Based Matching)\n", "header")
            ...
            if meal_type in nutritional_data['meal_targets']:
                ...
                self.targets_text.insert(tk.END, f"{self.meal_type_var.get()} Targets (MLP Input Vector):\n", "subheader")
            ...
            if health_conditions:
                self.targets_text.insert(tk.END, "Health Condition Adjustments (MLP Model Selection):\n", "subheader")
                for condition in health_conditions:
                    self.targets_text.insert(tk.END, f"• {condition}: Using specialized MLP neural network\n")
                self.targets_text.insert(tk.END, "\n")
            
            self.targets_text.insert(tk.END, "MLP Neural Network Process:\n", "subheader")
            self.targets_text.insert(tk.END, "1. Convert your targets into a feature vector\n")
            self.targets_text.insert(tk.END, "2. Use trained neural networks to predict health scores\n")
            self.targets_text.insert(tk.END, "3. Combine nutritional similarity with health predictions\n")
            self.targets_text.insert(tk.END, "4. Rank by combined MLP score (lower = better match)\n")
            ...
            self.status_var.set("Nutritional targets calculated - ready for MLP neural network matching")
            
        except Exception as e:
            messagebox.showerror("Error", f"Error calculating nutrition targets: {str(e)}")
    
    def get_recommendations(self):
        """Get MLP-based food recommendations"""
        try:
            ...
            self.status_var.set("Running MLP neural networks to find optimal food matches...")
            ...
            if recommendations:
                ...
                self.status_var.set(f"MLP found {len(recommendations)} optimal foods for {condition_text}")
            ...
            self.status_var.set("Error occurred during MLP recommendation")
    ...
    def show_food_details(self, event):
        ...
        if item_index < len(self.last_recommendations):
            ...
            # Add MLP analysis information
            self.details_text.insert(tk.END, "MLP Neural Network Analysis:\n", "header")
            self.details_text.insert(tk.END, f"• Combined MLP Score: {rec['mlp_score']:.4f} (lower = better match)\n")
            self.details_text.insert(tk.END, f"• Neural Network Model: {rec['model_type']}\n")
            self.details_text.insert(tk.END, f"• Predicted Health Score: {rec['predicted_health_score']:.3f}\n")
            ...
            if rec['mlp_score'] < 1.0:
                ...
                similarity = "Good"
                color = "good"
            elif rec['mlp_score'] < 3.0:
                similarity = "Fair"
                color = "warning"
            else:
                similarity = "Poor"
                color = "warning"
            self.details_text.insert(tk.END, f"• Match Quality: {similarity}\n\n", color)
            ...
            self.details_text.insert(tk.END, "Why This Food Was Recommended (MLP):\n", "header")
    ...
    def update_charts(self, recommendations):
        """Update visualization charts with MLP insights"""
        ...
        try:
            ...
            # Chart 2: MLP Score Distribution
            mlp_scores = [r['mlp_score'] for r in recommendations]
            self.ax2.hist(mlp_scores, bins=5, color=self.colors['secondary'], alpha=0.7, edgecolor='black')
            self.ax2.set_title('MLP Score Distribution', fontsize=10, fontweight='bold')
            self.ax2.set_xlabel('MLP Score (lower = better)')
...
def main():
    ...
    print("Starting Health-Driven MLP Food Recommendation System...")
    
    # Create main window
    root = tk.Tk()
    root.title("Health-Driven MLP Food Recommendation System")
    ...
    def create_splash():
        ...
        splash.title("Loading MLP System...")
        ...
        ttk.Label(splash_frame, text="Health-Driven MLP Food Recommendation System", 
                 font=('Segoe UI', 16, 'bold')).pack(pady=20)
        ttk.Label(splash_frame, text="Multi-layer Perceptron Neural Network + Medical Guidelines", 
        ...
        status_label = ttk.Label(splash_frame, text="Initializing MLP models...")
        status_label.pack()
    ...
    def initialize_system():
        try:
            ...
            recommender = HealthAwareMLPRecommender(update_splash_status)
            
            update_splash_status("Training MLP neural networks...")
            ...
            # Create main UI
            app = HealthDrivenMLPFoodRecommenderUI(root, recommender)
            ...
            print("MLP System initialized successfully!")
            
        except Exception as e:
            close_splash_safely()
            root.deiconify()  # Show main window even if there's an error
            messagebox.showerror("Initialization Error", f"Failed to initialize MLP system: {str(e)}")
            print(f"Error: {e}")
