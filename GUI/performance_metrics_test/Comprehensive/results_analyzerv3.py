...
class ModelResultsAnalyzer:
    ...
    def create_visualizations(self, output_dir="./analysis_output", show_plots=False):
        ...
        # 1. Model Performance Comparison (Original 4-subplot version)
        summary_df = self.get_performance_summary()
        
        fig, axes = plt.subplots(2, 2, figsize=(15, 12))
        fig.suptitle('Model Performance Comparison - Detailed View', fontsize=16, fontweight='bold')
        ...
        # 1b. Model Performance Comparison (NEW Combined single-plot version)
        ...
        ax.text(len(models)-0.5, 0.52, 'Baseline (0.5)', ha='right', va='bottom', 
               fontsize=9, style='italic', color='gray')
        
        plt.tight_layout()
        plt.savefig(f"{output_dir}/model_performance_comparison_combined.png", dpi=300, bbox_inches='tight')
        if show_plots:
            plt.show()
        plt.close()
